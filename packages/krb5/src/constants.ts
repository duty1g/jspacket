...
export function encodeFlags(flags: number[]): number[] {
  const finalFlags = new Array(32).fill(0);
  for (const f of flags) finalFlags[f] = 1;
  return finalFlags;
}

export enum ApplicationTagNumbers {
  Ticket = 1,
  Authenticator = 2,
  EncTicketPart = 3,
  AS_REQ = 10,
...
export enum PreAuthenticationDataTypes {
  PA_TGS_REQ = 1,
  PA_ENC_TIMESTAMP = 2,
  PA_PW_SALT = 3,
  PA_ENC_UNIX_TIME = 5,
...
export const KERB_NON_KERB_SALT = 16;
export const KERB_NON_KERB_CKSUM_SALT = 17;

export enum AuthorizationDataType {
  AD_IF_RELEVANT = 1,
...
export enum TransitedEncodingTypes {
  DOMAIN_X500_COMPRESS = 1,
}

export enum ProtocolVersionNumber {
  pvno = 5,
}

export enum KerberosMessageTypes {
  KRB_AS_REQ = 10,
  KRB_AS_REP = 11,
  KRB_TGS_REQ = 12,
  KRB_TGS_REP = 13,
...
export const KRB5_GSS_OID = '1.2.840.113554.1.2.2';

export const DIRECTIONAL_AP_REQ_SENDER = Buffer.alloc(4);
export const DIRECTIONAL_AP_REQ_RECIPIENT = Buffer.from([0, 0, 0, 1]);
